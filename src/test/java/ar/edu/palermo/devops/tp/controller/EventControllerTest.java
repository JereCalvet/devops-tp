...
public class EventControllerTest {
    ...
    void createEvent_WhenDataIsOk_ShouldReturnsCreatedEventAndLocationHeader() {
        ...
        try {
            mockMvc.perform(post(API_EVENTS_BASE_URL)
                    ...
                    .andExpect(header().string("Location", API_EVENTS_BASE_URL + "/" + eventId))
    ...
    @DisplayName("updateEvent returns bad request when input is invalid")
    @Test
    void updateEvent_WhenDataIsInvalid_ShouldReturnBadRequest() {
        // given
        final var eventId = 1L;
        final var invalidName = "AC";
        final var description = "Updated concert description";
        final var date = LocalDateTime.of(2025, 12, 10, 18, 0);

        final var eventDto = new EventDto(null, invalidName, description, date);
